...
import { VertexProperty } from '../../../common-tools/typing-tools/typing-tools';
...
export async function createQuestions(questions: QuestionData[]): Promise<void> {
   ...
   for (const question of questions) {
      traversal = traversal
         ...
         .coalesce(
            __.unfold(),
            __.addV('question').property('questionId', Number(question.questionId)),
         );
   }
   ...
   surplusQuestions = surplusQuestions.filter(
      q => questions.find(
         v => v.questionId === q.value
      ) == null,
   );
   ...
   await (traversal2 as gremlin.process.GraphTraversal).unfold().drop().iterate();
