...
const clientForStringQueries = new gremlin.driver.Client(databaseUrl, {
   traversalSource: "g",
...
export async function waitForDatabase(): Promise<void> {
   ...
   const checkDatabase = () => {
      ...
      if (attemptsAmount > showErrorsAfterAttemptsAmount) {
         console.log(
            `Database connection was attempted ${attemptsAmount} times and is not connecting yet, it seems to be taking long, it may be not working.`,
         );
      }
      g.inject(0)
         .toList()
...
export async function sendQueryAsString<T = any>(query: string): Promise<T> {
   ...
   try {
      ...
      console.log(`Error: ${tryToGetErrorMessage(error)}`);
      throw error;
   }
}
