...
describe("Cards game", () => {
    ...
    test("Users with matching tags appear first", async () => {
        ...
        await (0, models_3.subscribeToTagsPost)({ token: searcher.token, tagIds: [tagIds[0], tagIds[1]] });
    ...
    test("Liking users appears in the first places of the results", async () => {
        ...
        await (0, models_3.subscribeToTagsPost)({ token: searcher.token, tagIds: [tag.tagId] });
        /*
        ...
        for (const usr of nonLikingUsers) {
            await (0, models_3.subscribeToTagsPost)({ token: usr.token, tagIds: [tag.tagId] });
        }
        recommendations = await (0, models_1.recommendationsGet)({ token: searcher.token }, replacements_1.fakeCtx);
