...
describe("Cards game", () => {
    ...
    const searcherParams = {
        ...users_1.generateRandomUserProps(),
        name: "searcher user",
        profileDescription: "",
        birthDate: date_tools_1.fromAgeToBirthDate(32),
        height: 265,
    ...
    const compatibleParams = {
        ...
        birthDate: date_tools_1.fromAgeToBirthDate(30),
        targetAgeMin: 20,
        targetAgeMax: 40,
        targetDistance: 60,
    ...
    const incompatibleTooOld = {
        ...
        birthDate: date_tools_1.fromAgeToBirthDate(40),
    };
    const incompatibleTooYoung = {
        ...users_1.generateRandomUserProps(),
        ...compatibleParams,
        name: "age incompatible 2",
        birthDate: date_tools_1.fromAgeToBirthDate(18),
    };
    ...
    beforeAll(async () => {
        searcherUser = await users_1.createFakeUser(searcherParams);
        fakeUsers = await users_1.createMultipleFakeCustomUsers(allUsersData);
        recommendations = await models_1.recommendationsGet({ token: searcherUser.token }, replacements_1.fakeCtx);
    });
    ...
    test("Incompatible recommendations tests were done correctly", () => {
        reusable_tests_1.createdUsersMatchesFakeData(fakeUsers, allUsersData, true);
    });
    test("Recommendations returns correct users in correct order", async () => {
        ...
        await users_1.setAttraction(searcherUser, recommendations, user_1.AttractionType.Dislike);
        recommendations = await models_1.recommendationsGet({ token: searcherUser.token }, replacements_1.fakeCtx);
        expect(recommendations).toHaveLength(0);
    });
    test("Disliked users returns the correct data", async () => {
        recommendations = await models_1.dislikedUsersGet({ token: searcherUser.token }, replacements_1.fakeCtx);
        // Check amount
        expect(recommendations).toHaveLength(usersDataCompatible.length);
        // Check for duplication
        expect(recommendations[0].userId !== recommendations[1].userId).toBe(true);
        await queries_1.queryToRemoveUsers(fakeUsers);
    });
    test("Users gets notified of new cards when they request for it", async () => {
        await queries_1.queryToRemoveUsers(fakeUsers);
        let mainUser = await users_1.createFakeUser();
        const fakeCompatibleUsers = await users_1.createFakeCompatibleUsers(mainUser, 10);
        // The test is going to be done correctly
        mainUser = (await models_2.userGet({ token: mainUser.token }, replacements_1.fakeCtx));
        ...
        await models_1.notifyAllUsersAboutNewCards();
        mainUser = (await models_2.userGet({ token: mainUser.token }, replacements_1.fakeCtx));
        expect(mainUser.notifications.length).toBe(0);
        // Here user requests for notifications
        await models_2.userPost({ token: mainUser.token, props: { sendNewUsersNotification: 10 } }, replacements_1.fakeCtx);
        ...
        mainUser = (await models_2.userGet({ token: mainUser.token }, replacements_1.fakeCtx));
        expect(mainUser.notifications.length).toBe(1);
        await queries_1.queryToRemoveUsers([mainUser]);
        await queries_1.queryToRemoveUsers(fakeCompatibleUsers);
    });
    test("Users with matching tags appear first", async () => {
        ...
        const searcher = (await users_1.createFakeCompatibleUsers(adminUser, 1))[0];
        // Create 5 tags (only admins can create unlimited tags)
        for (let i = 0; i < 5; i++) {
            await models_3.createTagPost({
        ...
        // Store the tags
        const tags = await models_3.tagsCreatedByUserGet(adminUser.token);
        const tagIds = tags.map(t => t.tagId);
        // Create 5 users compatible but not with tags, so we can test these users will appear last
        await users_1.createFakeCompatibleUsers(searcher, 5);
        // Create 3 users that will be compatible in tags, so we can test that these users appear first
        const tagCompatibleUsers = await users_1.createFakeCompatibleUsers(searcher, 3);
        // Create a user that blocks the searcher, so we can test that this user also does not appear
        const userShouldNotAppear1 = (await users_1.createFakeCompatibleUsers(searcher, 1))[0];
        const userShouldNotAppear2 = (await users_1.createFakeCompatibleUsers(searcher, 1))[0];
        // Searcher user subscribes to 2 tags and blocks 1 tag
        await models_3.subscribeToTagsPost({ token: searcher.token, tagIds: [tagIds[0], tagIds[1]] });
        await models_3.blockTagsPost({ token: searcher.token, tagIds: [tagIds[2]] });
        // The user that should appear first does the same than the searcher
        await models_3.subscribeToTagsPost({
            token: tagCompatibleUsers[0].token,
            tagIds: [tagIds[0], tagIds[1]],
        });
        await models_3.blockTagsPost({ token: tagCompatibleUsers[0].token, tagIds: [tagIds[2]] });
        ...
        recommendations = await models_1.recommendationsGet({ token: searcher.token }, replacements_1.fakeCtx);
    ...
    test("Liking users appears in the first places of the results", async () => {
        const searcher = await users_1.createFakeUser();
        const nonLikingUsers = await users_1.createFakeCompatibleUsers(searcher, configurations_1.NON_SEARCHER_LIKING_CHUNK * 3);
        const likingUsers = await users_1.createFakeCompatibleUsers(searcher, configurations_1.SEARCHER_LIKING_CHUNK);
        for (const usr of likingUsers) {
            await models_2.setAttractionPost({
                token: usr.token,
                attractions: [{ userId: searcher.userId, attractionType: user_1.AttractionType.Like }],
            }, replacements_1.fakeCtx);
        }
        const tag = await models_3.createTagPost({
            token: searcher.token,
            name: `searcher tag`,
            category: "test category 1",
        }, replacements_1.fakeCtx);
        await models_3.subscribeToTagsPost({ token: searcher.token, tagIds: [tag.tagId] });
        /*
        ...
        for (const usr of nonLikingUsers) {
            await models_3.subscribeToTagsPost({ token: usr.token, tagIds: [tag.tagId] });
        }
        recommendations = await models_1.recommendationsGet({ token: searcher.token }, replacements_1.fakeCtx);
    ...
    afterAll(async () => {
        ...
        await models_3.removeAllTagsCreatedBy(testUsers);
    });
});
