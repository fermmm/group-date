...
async function createFakeUsersPost(params, ctx) {
    const user = await models_3.retrieveFullyRegisteredUser(params.token, false, ctx);
    if (!user.isAdmin) {
        return `Error: Only admins can use this`;
    }
    const usersCreated = await users_1.createFakeCompatibleUsers(await models_3.retrieveFullyRegisteredUser(params.token, false, ctx), Number(params.text));
    allUsersCreated.push(...usersCreated);
    for (let i = 0; i < usersCreated.length; i++) {
        const userCreated = usersCreated[i];
        if (i % 2 == 0) {
            await models_2.subscribeToTagsPost({ token: userCreated.token, tagIds: [configurations_1.APP_AUTHORED_TAGS[0].tagId] });
        }
        else {
            await models_2.subscribeToTagsPost({ token: userCreated.token, tagIds: [configurations_1.APP_AUTHORED_TAGS[1].tagId] });
        }
    }
    for (const userCreated of allUsersCreated) {
        await models_3.setAttractionPost({
...
async function createFakeTagsPost(params, ctx) {
    ...
    for (let i = 0; i < Number(params.text); i++) {
        const tagParams = {
            token: user.token,
            name: generalTools_1.chance.sentence({ words: general_1.getRandomInt(1, 3) }),
            category: generalTools_1.chance.sentence({ words: 1 }),
            creationDate: moment().subtract(i, "days").unix(),
            lastInteractionDate: moment().subtract(general_1.getRandomInt(0, i), "days").unix(),
            fakeSubscribersAmount: Math.random() < 0.5 ? i * 2 : general_1.getRandomInt(0, 5),
            fakeBlockersAmount: Math.random() < 0.5 ? Math.floor(i * 0.2) : 0,
        };
        tagsCreated.push(await models_2.createTagPost(tagParams, ctx));
    }
...
async function forceGroupSearch(params, ctx) {
    ...
    const groupsCreated = await models_1.searchAndCreateNewGroups();
...
async function createFakeChatConversation(params, ctx) {
    ...
    const groups = await models_4.userGroupsGet({ token: params.token }, ctx, true);
    let currentConversationMessage = 0;
    for (const group of groups) {
        for (const member of group.members) {
            const message = fakeConversation[currentConversationMessage];
            const chatUser = await data_conversion_1.fromQueryToUser(queries_1.queryToGetUserById(member.userId), true);
            await models_4.chatPost({ token: chatUser.token, groupId: group.groupId, message }, ctx);
