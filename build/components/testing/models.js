...
async function createFakeUsersPost(params, ctx) {
    ...
    for (let i = 0; i < usersCreated.length; i++) {
        ...
        if (i % 2 == 0) {
            await (0, models_2.subscribeToTagsPost)({ token: userCreated.token, tagIds: [configurations_1.APP_AUTHORED_TAGS[0].tagId] });
        }
        else {
            await (0, models_2.subscribeToTagsPost)({ token: userCreated.token, tagIds: [configurations_1.APP_AUTHORED_TAGS[1].tagId] });
        }
    }
