...
8. [ This is not working ] With the following command a scheduled task is added to the OS to keep certificates renewed, also the command includes other hook commands to execute during renewal to stop and start the server because it needs port 80 for a moment to validate domain, so run one of these commands:
